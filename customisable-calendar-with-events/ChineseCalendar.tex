% THIS FILE MUST BE COMPILED WITH XELATEX OR LUALATEX. Click on the Overleaf Menu icon above the file list and set "Compiler" to "LuaLaTeX"
...
\coverImage[实际印刷出来的台历效果。较小尺寸的（\texttt{small} 选项，9\,cm $\times$ 9.5\,cm）适用3.5寸软盘盒（家里有这些古董的可以派上用场了）。较大尺寸的（默认设定，11.7\,cm $\times$ 13.65\,cm）则适用CD盒。另外还有\texttt{giant}选项，一页A4纸就一个月历。]
{actual}
...
\event[mark style=holiday]{2020-01-01}{}{新年元旦}
%% This is a 5-day event
\event[mark style=blue icon,marker=\faBriefcase]{2020-01-30}{5}{ACME大会}
%% you could also write \event{2020-01-06}{2020-02-03}{ACME 大会}
...
{\huge\selectfont
\[%
\hat{f}(\xi) = \int^{\infty}_{-\infty} f(x) \mathrm{e}^{-2\pi ix\xi} \mathrm{d}x
\]}
...
\event[mark style=blue icon,marker=\faBriefcase]{2020-01-30}{5}{ACME大会}
\event[mark style=pink icon,marker=\faBirthdayCake]{2020-02-07}{}{朋友生日}
\event{2020-02-24}{}{死线！！}
...
\otherstuff{\linewidth}{\tikz[scale=1.5]\duck[overleaf];}
